...
export default function Reads() {
  ...
  return (
    <>
      <Navbar />
      <main className="pt-28 pb-40 min-h-screen bg-white text-gray-800 px-4 sm:px-6 md:px-10 font-sans dark:bg-[#1F1B16] dark:text-[#FAF4ED] transition-colors">
        <div className="max-w-6xl mx-auto">
          <h1 className="text-4xl font-bold mb-8 text-[#4B4032] dark:text-[#F9C06B]">Books I’ve Read in 2025</h1>

          {books.length === 0 ? (
            <p className="text-center text-gray-500 italic dark:text-gray-400 mt-6">
              🐾 No books added yet. Maybe I’ve been too busy reading code?
            </p>
          ) : (
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {books.map((book) => {
                ...
                return (
                  <div
                    key={book.id}
                    className="p-4 border border-[#F9C06B] bg-[#FEF7EC] dark:bg-[#2B2520] dark:border-[#F9C06B] rounded-xl shadow hover:shadow-lg transition text-center"
                  >
                    {coverUrl && (
                      <div className="w-full h-40 overflow-hidden rounded mb-3 bg-white dark:bg-[#3a332b] flex items-center justify-center">
                        <img
                         src={coverUrl}
                    ...
                    <h2 className="text-lg font-semibold mb-1 text-[#4B4032] dark:text-[#F9C06B]">{b.Title}</h2>
                    <p className="text-xs text-gray-500 mb-1 italic dark:text-gray-400">by {b.Author}</p>
                    <p className="text-yellow-500 text-sm mb-2">{renderStars(b.Rating)}</p>
                    {b.Review && (
                      <p className="text-sm text-gray-700 dark:text-gray-300 line-clamp-4">{b.Review}</p>
                    )}
                  </div>
                );
              })}
